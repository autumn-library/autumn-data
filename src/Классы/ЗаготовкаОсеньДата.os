...
Функция ИнициализироватьИсточникиДанных()
	...
	Для Каждого НастройкаИсточникаДанных Из НастройкиИсточниковДанных Цикл
		...
		РазмерПула = ПолучитьИли(НастройкаИсточникаДанных, "РазмерПула", 10);

		ИсточникДанных = Новый ИсточникДанных(ИмяИсточника, ТипКоннектора, СтрокаСоединения, ПараметрыКоннектора, РазмерПула);

		// Источником владеет поделка, а не поток, в котором ее собрали: поделку могут собирать в
		// потоке, который завершится раньше приложения, и закрытие источника по завершению этого
		// потока унесло бы соединения у живых потребителей.
		ИсточникДанных.УстановитьАвтоЗакрытие(Ложь);
...
Процедура ПриИнициализацииПоделки(Поделка) Экспорт
	...
	Для Каждого ИсточникДанных Из ИсточникиДанных Цикл
		...
		Поделка.ДобавитьЗавязь(
			...
			|(&Пластилин(""ТипыСущностей" + ИсточникДанных.Имя() + """) ТипыСущностей, &Пластилин(""НаблюдательИсточникаДанных"", Тип = ""Массив"") Наблюдатели) ->
			|	// Наблюдатели регистрируются на источнике до создания менеджера, чтобы видеть и создание таблиц.
			|	// Завязь наблюдателя вправе вернуть Неопределено - выключенное наблюдение пропускается.
			|	Для Каждого Наблюдатель Из Наблюдатели Цикл
			|		Если Наблюдатель <> Неопределено Тогда
			|			ИсточникДанных.ДобавитьНаблюдателя(Наблюдатель);
			|		КонецЕсли;
			|	КонецЦикла;
			|
			|	МенеджерСущностей = Новый МенеджерСущностей(ИсточникДанных);
			|
		...
		Для Каждого ТипСущности Из ТипыСущностей Цикл
			...
			Поделка.ДобавитьЗавязь(
				...
				|&Прозвище(""ХранилищеСущностей" + ИсточникДанных.Имя() + """)

...
// Соответствие - Размер пула соединений по источнику данных: ИсточникДанных -> Число.
// ИсточникДанных entity размер пула не хранит, а менеджеру он нужен при создании.
Перем РазмерыПулов;

Функция ИнициализироватьИсточникиДанных()

	ИсточникиДанных = Новый Массив;
	РазмерыПулов = Новый Соответствие;
	...
	Для Каждого НастройкаИсточникаДанных Из НастройкиИсточниковДанных Цикл
		...
		РазмерыПулов.Вставить(ИсточникДанных, РазмерПула);
...
Процедура ПриИнициализацииПоделки(Поделка) Экспорт
	...
	Для Каждого ИсточникДанных Из ИсточникиДанных Цикл
		...
		Поделка.ДобавитьЗавязь(
			...
			Новый Структура("ИсточникДанных, РазмерПула", ИсточникДанных, РазмерыПулов[ИсточникДанных])
		);

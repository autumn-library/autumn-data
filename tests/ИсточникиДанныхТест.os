...
&Тест
&Изолированный(Совет = "() -> ГенераторДанных.СоветДругогоМастераСОднимКоннекторомКакСоответствие()")
Процедура КогдаИсточникСоздалаПоделка_ТогдаОнНеЗакрываетсяСПотокомСоздания() Экспорт

	// Дано-Когда
	ИсточникДанных = Поделка.НайтиЖелудь("ИсточникДанных");

	// Тогда: источник живет столько же, сколько поделка, а собрать ее могли в потоке, который
	// завершится раньше приложения - автозакрытие унесло бы соединения у живых потребителей.
	Ожидаем.Что(ИсточникДанных.АвтоЗакрытие(), "Источником владеет поделка, а не создавший ее поток")
		.Равно(Ложь);

...
Завязь с таким прозвищем вправе вернуть `Неопределено` - например, когда наблюдение выключено настройкой: autumn не кладет такой желудь в коллекцию, и наблюдатель не регистрируется. Без желудей с прозвищем источники остаются без наблюдателей, а менеджеры создаются как раньше.

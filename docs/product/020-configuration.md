...
Менеджер сущностей entity сообщает наблюдателям о своей работе: операциях, запросах к СУБД, соединениях пула и транзакциях, см. [наблюдателей сущностей](https://github.com/nixel2007/entity/blob/develop/docs/Наблюдатели.md) в entity. `autumn-data` регистрирует наблюдателей сама: каждый желудь с прозвищем `НаблюдательСущностей` подставляется во все менеджеры сущностей до их инициализации, поэтому наблюдатель видит и создание таблиц.
...
На этом построена интеграция с OpenTelemetry: библиотека `autumn-opentelemetry` объявляет завязь наблюдателя `ОтелНаблюдательСущностей` с этим прозвищем, и трассировка операций и запросов включается без единой строки в прикладном коде.

## Доступ к коннектору

В entity 4 коннектор принадлежит пулу менеджера, и метод `ПолучитьКоннектор` удален. Вместо него менеджер и хранилища выполняют алгоритм на занятом соединении: `ВыполнитьСКоннектором(Алгоритм, Контекст)` и `ВычислитьСКоннектором(Алгоритм, Контекст)`, где алгоритм - лямбда с параметром `Коннектор`.

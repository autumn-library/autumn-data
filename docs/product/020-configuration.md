...
Параметр `РазмерПула` задает предел одновременных соединений с БД. Если не задан, пул вмещает 10 соединений. Ноль отключает переиспользование соединений: каждое обращение к БД открывает свое соединение и закрывает его по завершении.
...
Коннектор принадлежит соединению, а не прикладному коду. Менеджер и хранилища выполняют алгоритм на занятом соединении: `ВыполнитьСКоннектором(Алгоритм, Контекст)` и `ВычислитьСКоннектором(Алгоритм, Контекст)`, где алгоритм - лямбда с параметром `Коннектор`.

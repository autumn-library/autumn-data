...
Параметр `РазмерПула` источника данных задает размер пула соединений его менеджера сущностей. Если не задан, пул вмещает 10 соединений. Ноль снимает предел: каждое обращение к БД открывает свое соединение и закрывает его по завершении.

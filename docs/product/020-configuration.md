...
Параметр `РазмерПула` задает предел одновременных соединений с источником данных. Если не задан, пул вмещает 10 соединений. Ноль отключает переиспользование соединений: каждое обращение к БД открывает свое соединение и закрывает его по завершении.

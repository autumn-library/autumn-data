...
- **Пул соединений и наблюдатели сущностей** - размер пула задается в настройках источника, а желуди с прозвищем `НаблюдательСущностей` подставляются во все менеджеры: на этом построена интеграция с OpenTelemetry в `autumn-opentelemetry`

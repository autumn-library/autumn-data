...
- **Пул соединений** - каждый менеджер сущностей работает через пул соединений entity 4, его размер задается в настройках источника данных
- **Наблюдатели источника данных** - желуди с прозвищем `НаблюдательИсточникаДанных` регистрируются на всех источниках данных: на этом построена интеграция с OpenTelemetry в `autumn-opentelemetry`
